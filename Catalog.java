...
public class Catalog
{
    ...
    public Catalog(ArrayList <CatalogItem> items) throws IllegalArgumentException{
        ...
        for (int x = 0; x < items.size(); x ++){
            try{
                int check = Integer.parseInt(items.get(x).getId());
                if (check > nextId){
                    nextId = check + 1;
                }
        ...
        if(!catalog.checkId(items)){
            throw new IllegalArgumentException("IDs must be unique");
        }
    }

    private boolean checkId(ArrayList <CatalogItem> items){
        boolean unique = true;
        for( int x =0; x < items.size(); x++){
            String checkID = items.get(x).getId();
            for (int y = (x+1); y < items.size(); y++){
                if(checkID.equals(items.get(y).getId())){
                    unique = false;
                }
            }
        }
     ...
     */
    public String add(LibraryItem lib_item)
    {
        nextId++;
        String bookId = String.valueOf(nextId);
        addBookWithId(lib_item, bookId);
        return bookId;
    }
    ...
    public ArrayList<String> search(String searchTerm)
    {
        ...
        {
            LibraryItem lib_item = item.getMyItem();

            if (lib_item.matches(searchTerm)){
    ...
    private void addBookWithId(LibraryItem lib_item, String id)
    {
        CatalogItem item = new CatalogItem(lib_item, id, true);
